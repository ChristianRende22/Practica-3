...
export const MovieDetails = ({ selectedId, onCloseMovie, onAddWatched, watched}) => {
    ...
    // hook personalizado para detalles de la peli
    const {movie, error, isLoading} = useFetchMovieDetails(selectedId);

    // extraemos info relevante
    const {
        Title: title,
        Year: year,
        Poster: poster,
    ...
    // verificar si ya está la peli
    const isWatched = watched.some(movie => movie.imdbID === selectedId);

    // obtiene cali previa
    const watchedUserRating = watched.find(movie => movie.imdbID === selectedId)?.userRating;

    // MANEJO DE ADICIÓN DE UNA PELI
    function handleAdd() {
        const newMovie = {
            imdbID: selectedId,
            title,
            ...
            runtime: Number(runtime.split(" ")[0]), // solo # de mins
            userRating
        };
